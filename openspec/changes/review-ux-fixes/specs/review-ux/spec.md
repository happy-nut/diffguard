...
### Requirement: 드래그 후 코멘트의 선택 잔상 제거
드래그(텍스트 선택) 후 코멘트를 열면, 컴포저나 코멘트 카드에 텍스트 선택 하이라이트가 남아서는 안 된다(MUST NOT).

#### Scenario: 드래그 후 컴포저에 선택 효과 없음
- **WHEN** 사용자가 diff에서 텍스트를 드래그 선택한 뒤 `?` 또는 `>`로 코멘트를 열 때
- **THEN** 선택한 코드는 코멘트에 캡처되지만 컴포저/카드에는 선택 하이라이트가 남지 않는다
